...
db_connection = pymysql.connect(
    ...
    password='usr',
    database='pipeline',
    cursorclass=pymysql.cursors.DictCursor  # 결과를 딕셔너리 형태로 가져옵니다.
)
...
def crawl_and_store_data():
    ...
    for page in range(1, final_page + 1):
        ...
        # 스레딩 로컬을 사용하여 각 스레드별로 데이터베이스 연결을 관리합니다.
        db_conn = db_connections.__dict__.get('connection')
        if db_conn is None:
            db_conn = db_connections.connection = pymysql.connect(
                host='localhost',
                user='root',
                password='usr',
                database='pipeline',
